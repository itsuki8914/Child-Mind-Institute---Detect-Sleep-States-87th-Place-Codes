...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq, scaler=None, grad_acc_step=1):
    model.train()
    ...
    for idx, (images, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        losses = losses / grad_acc_step
        if scaler is not None:
            scaler.scale(losses).backward()
            if (idx+1)%grad_acc_step==0:
